...
func CalibrateSolarScaleFactor(
	...
	if maxSolarKWH > 1.0 { // only consider clipping if production is significant
		...
		// only if we've seen this for at least 3 hours
	...
	log.Ctx(ctx).DebugContext(
		ctx,
		"calculated daily static scale factor",
	...
	for h := 0; h < 24; h++ {
		...
		if isSignificantDifference {
			log.Ctx(ctx).DebugContext(
				ctx,
				"solar scale factor hour invalid (significantly different/outlier)",
				slog.Int("hourOfDay", h),
	...
	if len(validHours) >= 4 {
		...
		log.Ctx(ctx).DebugContext(
			ctx,
			"fallback to static scale factor (not enough valid hours)",
