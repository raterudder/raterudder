...
func TestSolve15mClippingCap(t *testing.T) {
	t.Run("no clipping needed", func(t *testing.T) {
		u := [4]float64{0.5, 0.5, 0.5, 0.5}
		c := solve15mClippingCap(u, 2.0)
		assert.InDelta(t, 0.5, c, 0.001)
	})

	t.Run("clipping applied to one chunk", func(t *testing.T) {
		u := [4]float64{2.0, 0.5, 0.5, 0.5}
		// Target is 2.5. If cap is 1.0, sum of min(u[i], 1.0) is:
		// min(2.0, 1.0) + 0.5 + 0.5 + 0.5 = 1.0 + 1.5 = 2.5.
		c := solve15mClippingCap(u, 2.5)
		assert.InDelta(t, 1.0, c, 0.001)
	})

	t.Run("clipping applied to all chunks", func(t *testing.T) {
		u := [4]float64{2.0, 2.0, 2.0, 2.0}
		// Target is 4.0. c should be 1.0.
		c := solve15mClippingCap(u, 4.0)
		assert.InDelta(t, 1.0, c, 0.001)
	})
}
...
func Test15mInverterClipping(t *testing.T) {
	...
	t.Run("15m clipping calibration and chunk-level clipping", func(t *testing.T) {
		// We have 7 days of history.
		// On each day, at hour 18 (noon Chicago time), the actual solar production is exactly 2.3 kWh,
		// but the unclipped predictions (due to clear skies) are:
		// chunk 1: 1.923 kWh, chunks 2,3,4: 0.192 kWh.
		// Sum of unclipped is 2.5.
		// The actual is 2.3, which is explained by a 15-minute clipping cap of ~1.72 kWh.
		history := []types.EnergyStats{}
		...
		for d := 0; d < 7; d++ {
			...
			// Add unclipped calibration hours 14, 15, 16, 17
			for h := 14; h <= 17; h++ {
				tsHour := day.Add(time.Duration(h) * time.Hour)
				weatherHours = append(weatherHours, types.HourlyWeather{
					TSHourStart:  tsHour,
					DHI:          100.0,
				...
				history = append(history, types.EnergyStats{
					TSHourStart:   tsHour,
					SolarKWH:      0.96, // Calibrates static panel efficiency to ~0.00782
					GridExportKWH: 1.0,
				})
			}

			// Add clipped hour 18
			tsHour := day.Add(18 * time.Hour)
			weatherHours = append(weatherHours, types.HourlyWeather{
				TSHourStart:  tsHour,
				DHI:          260.0,
				TemperatureC: -100.0,
				Forecast15m: []types.Weather15m{
					{MinuteStart: 0, DHI: 800.0},
					{MinuteStart: 15, DHI: 80.0},
					{MinuteStart: 30, DHI: 80.0},
					{MinuteStart: 45, DHI: 80.0},
				},
			})

			history = append(history, types.EnergyStats{
				TSHourStart:   tsHour,
				SolarKWH:      2.3, // Clipped value
				GridExportKWH: 1.0,
			})
		}
		...
		val18 := res[baseDate.Add(18*time.Hour).Unix()]
		assert.InDelta(t, 2.3, val18.ImprovedSolar, 0.1)
		assert.InDelta(t, 2.5, val18.UnclippedSolar, 0.1)
	})
}
