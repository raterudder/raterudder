...
func TestRockyMountainPowerRates(t *testing.T) {
	...
	t.Run("Utah Residential Service flat rate with Net Billing Schedule 137", func(t *testing.T) {
		...
		targetSummer := time.Date(2026, time.July, 15, 14, 0, 0, 0, denver)
		...
		targetWinter := time.Date(2026, time.December, 15, 14, 0, 0, 0, denver)
	...
	t.Run("Utah Residential Service TOU with Net Billing Schedule 137", func(t *testing.T) {
		...
		targetSummerPeak := time.Date(2026, time.July, 15, 19, 0, 0, 0, denver)
	...
	t.Run("Idaho Residential Service with Net Billing Schedule 136", func(t *testing.T) {
		...
		targetSummerPeak := time.Date(2026, time.July, 15, 16, 0, 0, 0, denver)
		pSummerPeak, err := u.priceForTime(targetSummerPeak)
		require.NoError(t, err)
		...
		targetSummerOff := time.Date(2026, time.July, 15, 14, 0, 0, 0, denver)
		...
		targetWinterPeak := time.Date(2026, time.November, 15, 19, 0, 0, 0, denver)
		...
		// Summer 2027 check
		targetSummer2027 := time.Date(2027, time.July, 15, 16, 0, 0, 0, denver)
		pSummer2027, err := u.priceForTime(targetSummer2027)
		require.NoError(t, err)
		assert.InDelta(t, 0.100048, pSummer2027.DollarsPerKWH, 1e-6)

		// Winter 2027 check
		targetWinter2027 := time.Date(2027, time.November, 15, 19, 0, 0, 0, denver)
	...
	t.Run("Wyoming Residential Service with Net Metering Schedule 135", func(t *testing.T) {
		...
		target := time.Date(2026, time.July, 15, 14, 0, 0, 0, denver)
