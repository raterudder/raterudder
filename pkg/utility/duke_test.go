...
func TestDuke(t *testing.T) {
	...
	t.Run("isDEIWinter identifies winter boundaries", func(t *testing.T) {
		...
		assert.True(t, isDEIWinter(time.Date(2026, time.January, 15, 12, 0, 0, 0, loc)))
		assert.True(t, isDEIWinter(time.Date(2026, time.February, 28, 12, 0, 0, 0, loc)))
		assert.True(t, isDEIWinter(time.Date(2026, time.March, 7, 12, 0, 0, 0, loc)))
		assert.False(t, isDEIWinter(time.Date(2026, time.March, 8, 12, 0, 0, 0, loc)))
		assert.False(t, isDEIWinter(time.Date(2026, time.June, 15, 12, 0, 0, 0, loc)))
		assert.False(t, isDEIWinter(time.Date(2026, time.October, 31, 12, 0, 0, 0, loc)))
		assert.True(t, isDEIWinter(time.Date(2026, time.November, 1, 12, 0, 0, 0, loc)))
		assert.True(t, isDEIWinter(time.Date(2026, time.December, 25, 12, 0, 0, 0, loc)))
	})
	...
	t.Run("duke_carolinas_nc_rs and re rates", func(t *testing.T) {
		...
		p := getPriceForTime(periods, time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
		...
		pRE := getPriceForTime(periodsRE, time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
	...
	t.Run("duke_carolinas_nc_rt seasons and hours", func(t *testing.T) {
		...
		pOn := getPriceForTime(periods, time.Date(2026, time.June, 15, 19, 0, 0, 0, loc))
		assert.Equal(t, 0.171204, pOn.DollarsPerKWH)

		// Discount: 1:00 - 6:00 (1 AM - 6 AM)
		pDisc := getPriceForTime(periods, time.Date(2026, time.June, 15, 2, 0, 0, 0, loc))
		assert.Equal(t, 0.053929, pDisc.DollarsPerKWH)

		// Off-peak: other (1 PM is off-peak in summer)
		pOff13 := getPriceForTime(periods, time.Date(2026, time.June, 15, 13, 0, 0, 0, loc))
		assert.Equal(t, 0.078411, pOff13.DollarsPerKWH)

		// Off-peak: other
		pOff := getPriceForTime(periods, time.Date(2026, time.June, 15, 8, 0, 0, 0, loc))
		assert.Equal(t, 0.078411, pOff.DollarsPerKWH)

		// Summer Weekend (June 14, 2026 - Sunday) - no on-peak
		pOnWE := getPriceForTime(periods, time.Date(2026, time.June, 14, 19, 0, 0, 0, loc))
		assert.Equal(t, 0.078411, pOnWE.DollarsPerKWH)

		// Winter Weekday (Dec 15, 2026 - Tuesday)
		// On-peak: 6:00 - 9:00
		pWinOn := getPriceForTime(periods, time.Date(2026, time.December, 15, 7, 0, 0, 0, loc))
	...
	t.Run("duke_carolinas_nc_rt_ev charging hours", func(t *testing.T) {
		...
		pOff := getPriceForTime(periods, time.Date(2026, time.June, 15, 2, 0, 0, 0, loc))
		assert.Equal(t, 0.061752, pOff.DollarsPerKWH)

		// Standard: other
		pStd := getPriceForTime(periods, time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
	...
	t.Run("duke_carolinas_nc export credits", func(t *testing.T) {
		...
		p := getPriceForTime(periodsRSC, time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
		...
		pPrem := getPriceForTime(periodsSCG, time.Date(2026, time.June, 15, 19, 0, 0, 0, loc))
		...
		pOn := getPriceForTime(periodsSCG, time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
		assert.Equal(t, 0.0479, pOn.GenerationCreditDollarsPerKWH)

		// Summer Off-Peak (other) => 0.0414
		pOff := getPriceForTime(periodsSCG, time.Date(2026, time.June, 15, 8, 0, 0, 0, loc))
	...
	t.Run("duke_carolinas_sc_r_stou seasons and hours", func(t *testing.T) {
		...
		pSuperOff := getPriceForTime(periods, time.Date(2026, time.June, 15, 3, 0, 0, 0, loc))
		assert.Equal(t, 0.093782, pSuperOff.DollarsPerKWH)

		// Off-peak: other
		pOff := getPriceForTime(periods, time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
		assert.Equal(t, 0.128191, pOff.DollarsPerKWH)

		// Winter weekday (Dec 15, 2026 - Tuesday)
		// On-peak: 6:00 - 9:00, 18:00 - 21:00
		pWinOn1 := getPriceForTime(periods, time.Date(2026, time.December, 15, 7, 0, 0, 0, loc))
		assert.Equal(t, 0.209021, pWinOn1.DollarsPerKWH)
		pWinOn2 := getPriceForTime(periods, time.Date(2026, time.December, 15, 19, 0, 0, 0, loc))
		assert.Equal(t, 0.209021, pWinOn2.DollarsPerKWH)

		// No super off-peak in winter (3 AM off-peak rate)
		pWinOff := getPriceForTime(periods, time.Date(2026, time.December, 15, 3, 0, 0, 0, loc))
	...
	t.Run("duke_progress_nc export rates (Schedule PP SCG)", func(t *testing.T) {
		...
		pPrem := getPriceForTime(periods, time.Date(2026, time.June, 15, 19, 0, 0, 0, loc))
		assert.True(t, pPrem.SeparateGenerationCredit)
		assert.Equal(t, 0.0597, pPrem.GenerationCreditDollarsPerKWH)

		// Summer On-Peak (14:00 - 18:00)
		pOn := getPriceForTime(periods, time.Date(2026, time.June, 15, 15, 0, 0, 0, loc))
	...
	t.Run("duke_progress_sc export rates (Schedule PP SCG with integration charge)", func(t *testing.T) {
		...
		pOn := getPriceForTime(periods, time.Date(2026, time.June, 15, 14, 0, 0, 0, loc))
	...
	t.Run("duke_indiana_rs_tou rate and export options", func(t *testing.T) {
		...
		pOn := getPriceForTime(periods, time.Date(2026, time.June, 15, 18, 0, 0, 0, loc))
		...
		pWinOn2 := getPriceForTime(periods, time.Date(2026, time.December, 15, 18, 0, 0, 0, loc))
		assert.Equal(t, 0.214198, pWinOn2.DollarsPerKWH)

		// Weekend/Holiday (Christmas Day - Dec 25, 2026 - Friday)
		// No on-peak: 6 pm is off-peak
		pHolOff := getPriceForTime(periods, time.Date(2026, time.December, 25, 18, 0, 0, 0, loc))
		assert.Equal(t, 0.142799, pHolOff.DollarsPerKWH)

		// Export rate check (EDG)
		pExp := getPriceForTime(periods, time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
		...
		pNet := getPriceForTime(periodsNet, time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
	...
	t.Run("Applying Settings via genericTOU", func(t *testing.T) {
		...
		p, err := u.priceForTime(time.Date(2026, time.June, 15, 12, 0, 0, 0, loc))
		require.NoError(t, err)
		assert.Equal(t, 0.122603, p.DollarsPerKWH)
		assert.True(t, p.SeparateGenerationCredit)
		assert.Equal(t, 0.0453, p.GenerationCreditDollarsPerKWH)
