...
func TestBWPUtility(t *testing.T) {
	bwpLoc, err := time.LoadLocation("America/Los_Angeles")
	...
	t.Run("Residential Flat 16.01 under 300 kWh", func(t *testing.T) {
		...
		p, err := u.priceForTime(time.Date(2026, time.June, 15, 12, 0, 0, 0, bwpLoc))
	...
	t.Run("Residential TOU EV R-TOU-EV Pricing and Seasons", func(t *testing.T) {
		...
		p, err := u.priceForTime(time.Date(2026, time.June, 15, 17, 0, 0, 0, bwpLoc)) // Monday
		if assert.NoError(t, err) {
		...
		p, err = u.priceForTime(time.Date(2026, time.June, 15, 4, 0, 0, 0, bwpLoc))
		if assert.NoError(t, err) {
			assert.InDelta(t, 0.1555, p.DollarsPerKWH, 1e-6)
		}

		// 4. Summer Weekend (Sunday is entirely Off-Peak)
		p, err = u.priceForTime(time.Date(2026, time.June, 14, 12, 0, 0, 0, bwpLoc)) // Sunday
		if assert.NoError(t, err) {
			assert.InDelta(t, 0.1555, p.DollarsPerKWH, 1e-6)
		}
		...
		p, err = u.priceForTime(time.Date(2026, time.December, 15, 12, 0, 0, 0, bwpLoc)) // Tuesday
		if assert.NoError(t, err) {
			assert.InDelta(t, 0.2617, p.DollarsPerKWH, 1e-6)
		}

		// 6. Winter Off-Peak (11:00 PM - 8:00 AM, weekday)
		p, err = u.priceForTime(time.Date(2026, time.December, 15, 4, 0, 0, 0, bwpLoc))
		if assert.NoError(t, err) {
			assert.InDelta(t, 0.1555, p.DollarsPerKWH, 1e-6)
		}

		// 7. Winter Weekend (entirely Off-Peak)
		p, err = u.priceForTime(time.Date(2026, time.December, 13, 12, 0, 0, 0, bwpLoc)) // Sunday
		if assert.NoError(t, err) {
			assert.InDelta(t, 0.1555, p.DollarsPerKWH, 1e-6)
		}
	})
	...
	t.Run("Residential TOU EV Holiday Sunday Shifting", func(t *testing.T) {
		...
		p, err := u.priceForTime(time.Date(2027, time.July, 5, 12, 0, 0, 0, bwpLoc)) // Monday 12 PM (usually Mid-Peak)
		if assert.NoError(t, err) {
	...
	t.Run("Residential TOU EV Export Schemes", func(t *testing.T) {
		...
		p, err := u.priceForTime(time.Date(2026, time.June, 15, 17, 0, 0, 0, bwpLoc))
		if assert.NoError(t, err) {
			assert.InDelta(t, 0.3902, p.DollarsPerKWH, 1e-6)
			assert.True(t, p.SeparateGenerationCredit)
			assert.InDelta(t, 0.0455, p.GenerationCreditDollarsPerKWH, 1e-6)
