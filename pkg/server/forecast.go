...
func (s *Server) handleForecast(w http.ResponseWriter, r *http.Request) {
	...
	if settings.Release == "staging" || s.devProxy != "" {
...
//  1. Calibrating a peak efficiency factor from historical actual solar vs. irradiance data.
...
func calculateImprovedSolar(ctx context.Context, history []types.EnergyStats, weather []types.Weather) map[int64]improvedSolar {
	...
	actualSolarByHour := make(map[int64]float64, len(history))
	for _, h := range history {
		actualSolarByHour[h.TSHourStart.Unix()] = h.SolarKWH
	}
	...
	for _, ts := range timestamps {
		...
		// We look for the historical hour whose ratio of
		//   actual_kWh / (GTI × TempFactor × SnowFactor)
		// is highest. This ratio captures panel capacity and any systematic
		// model bias (tilt / azimuth correction, soiling, etc.).
		// Only hours with meaningful irradiance and actual generation
		// are considered to avoid noise from dawn/dusk readings.
		if actual, ok := actualSolarByHour[ts]; ok {
			if actual > 0.1 && gti >= 50 && h.TempFactor > 0 && h.SnowFactor > 0 {
				eff := actual / (gti * h.TempFactor * h.SnowFactor)
				if eff > maxEfficiency {
					maxEfficiency = eff
					maxEfficiencyTS = hw.TSHourStart
				}
			}
		}
	...
	log.Ctx(ctx).DebugContext(
		ctx,
		"calculated max efficiency",
		slog.Float64("maxEfficiency", maxEfficiency),
		slog.Time("maxEfficiencyTS", maxEfficiencyTS),
	)

	// 3. Project solar generation for every weather hour using the calibrated peak
	// efficiency. The formula is the inverse of the calibration.
	if maxEfficiency > 0 {
		for _, ts := range timestamps {
			h := results[ts]
			if h.GTI > 0 {
				h.ImprovedSolar = h.GTI * maxEfficiency * h.TempFactor * h.SnowFactor
			}
			results[ts] = h
		}
	}
