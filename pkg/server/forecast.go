...
func (s *Server) handleForecast(w http.ResponseWriter, r *http.Request) {
	...
	if settings.Location != nil && len(weatherHistory) > 0 {
		...
		if err == nil {
			...
			tomorrowStart := todayMidnight.AddDate(0, 0, 1)
			tomorrowEnd := todayMidnight.AddDate(0, 0, 2)

			solar15mMap := controller.CalculateWeatherSolar15m(ctx, now, energyHistory, weatherHistory, *settings.Location)
			...
			for _, hw := range allForecastHours {
				if !hw.TSHourStart.Before(tomorrowStart) && hw.TSHourStart.Before(tomorrowEnd) {
