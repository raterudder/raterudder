...
func TestAuthMiddleware(t *testing.T) {
	...
	t.Run("Multi Site Mode - Auth but No SiteID", func(t *testing.T) {
		...
		// Mock GetUser
		// Ensure a valid context is passed to the mock rather than using mock.Anything
		mocks.On("GetUser", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), "google:user@example.com").Return(types.User{
	...
	t.Run("Multi Site Mode - Auth and Valid SiteID (Query Param)", func(t *testing.T) {
		...
		// Ensure a valid context is passed to the mock rather than using mock.Anything
		mocks.On("GetSite", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), "site1").Return(types.Site{
			ID: "site1",
	...
	t.Run("Multi Site Mode - Auth and Invalid SiteID", func(t *testing.T) {
		...
		// Ensure a valid context is passed to the mock rather than using mock.Anything
		mocks.On("GetSite", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), "site3").Return(types.Site{
			ID:          "site3",
			Permissions: []types.SitePermissions{},
		}, nil).Once()
	...
	t.Run("Multi Site Mode - Auth and POST Body SiteID", func(t *testing.T) {
		...
		// Ensure a valid context is passed to the mock rather than using mock.Anything
		mocks.On("GetSite", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), "site2").Return(types.Site{
			ID: "site2",
			Permissions: []types.SitePermissions{
				{UserID: "google:user@example.com"},
			},
	...
	t.Run("Multi Site Mode - Join with New User", func(t *testing.T) {
		...
		// Ensure a valid context is passed to the mock rather than using mock.Anything
		mocks.On("GetUser", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), "google:user@example.com").Return(types.User{}, storage.ErrUserNotFound).Once()
	...
	t.Run("Multi Site Mode - User Lookup Fails", func(t *testing.T) {
		...
		// Ensure a valid context is passed to the mock rather than using mock.Anything
		mocks.On("GetUser", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), "google:user@example.com").Return(types.User{}, assert.AnError).Once()
