...
func TestAuthMiddleware(t *testing.T) {
	...
	t.Run("Multi Site Mode - Auth but No SiteID", func(t *testing.T) {
		...
		// Mock GetUser
		mocks.On("GetUser", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), // Ensure a non-nil valid context is passed
			"google:user@example.com").Return(types.User{
	...
	t.Run("Multi Site Mode - Auth and Valid SiteID (Query Param)", func(t *testing.T) {
		...
		mocks.On("GetSite", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), // Ensure a non-nil valid context is passed
			"site1").Return(types.Site{
			ID: "site1",
	...
	t.Run("Multi Site Mode - Join with New User", func(t *testing.T) {
		...
		mocks.On("GetUser", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), // Ensure a non-nil valid context is passed
			"google:user@example.com").Return(types.User{}, storage.ErrUserNotFound).Once()
	...
	t.Run("Multi Site Mode - User Lookup Fails", func(t *testing.T) {
		...
		mocks.On("GetUser", mock.MatchedBy(func(ctx context.Context) bool { return ctx != nil }), // Ensure a non-nil valid context is passed
			"google:user@example.com").Return(types.User{}, assert.AnError).Once()
