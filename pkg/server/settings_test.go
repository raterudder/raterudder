...
func TestSettings(t *testing.T) {
	...
	t.Run("Update Settings - Backfills History on New Username/Password", func(t *testing.T) {
		srv, mockES := newAuthServer("my-audience", []string{"admin@example.com"}, nil)
		...
		// Expect Authenticate to be called with the provided credentials
		// Note: No longer merged, pairing new username with provided password
		mockES.On("Authenticate", mock.Anything, mock.MatchedBy(func(c types.Credentials) bool {
			return c.Franklin != nil && c.Franklin.Username == "foo" && c.Franklin.Password == "bar" && c.Franklin.Token == ""
		})).Return(types.Credentials{
			Franklin: &types.FranklinCredentials{Username: "foo", MD5Password: "bar", GatewayID: "gw-123", Password: "bar"},
		}, true, nil)
