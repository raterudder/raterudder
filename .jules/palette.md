...
## 2024-05-24 - Screen Reader Support for Dynamic Messages
**Learning:** Conditionally rendered status messages (like `{error && <div>{error}</div>}` or `{successMessage && <div>{successMessage}</div>}`) are entirely invisible to screen readers unless they have the appropriate ARIA roles. Focus doesn't naturally shift to them when they appear.
**Action:** Always add `role="alert"` for error messages and `role="status"` for success/info messages when they are dynamically rendered in the UI to ensure screen readers announce them.
