...
## 2024-05-27 - Group interactive controls and announce dynamic text
**Learning:** When text (like the current date) updates dynamically, screen readers won't announce the change unless `aria-live="polite"` is used. Grouping the related navigation buttons with `role="group"` and `aria-label` provides better context.
**Action:** Always use `aria-live="polite"` on non-interactive elements that dynamically update text so screen readers announce the change without stealing focus. Group related interactive controls using `role="group"` and a descriptive `aria-label`.
