...
## 2024-05-27 - Announce date changes with aria-live="polite"
**Learning:** When using previous/next buttons to change dates, the DOM updates dynamically without a page refresh. Screen readers may not announce the new date unless explicitly told to do so, leaving users confused if the change was successful.
**Action:** Add `aria-live="polite"` and `aria-atomic="true"` to non-interactive elements that dynamically update text (like the current date label). Additionally, group related interactive controls using `role="group"` and a descriptive `aria-label` (e.g., `aria-label="Date navigation"`).
