...
## 2026-04-06 - Adding loading states to asynchronous buttons
**Learning:** Found an opportunity to improve the user experience by adding visual loading indicators (spinners) to buttons performing async tasks (e.g., date selection fetching, feedback submission). Users are likely to be confused if they click a button and nothing visually indicates that their request is processing, especially if the request takes a few seconds. The application already had a standard `<span className="loading-spinner" aria-hidden="true"></span>` pattern for this.
**Action:** Always ensure that buttons triggering asynchronous actions provide clear visual feedback, typically using a standard loading spinner combined with disabling the button to prevent duplicate submissions.
