...
interface HistoryDataPoint extends EnergyStats {
    ...
    solar15mImproved?: number;
    solar15mUnclipped?: number;
}
...
const History: React.FC<{ siteID?: string }> = ({ siteID }) => {
    ...
    const [hasSolar15m, setHasSolar15m] = useState(false);
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const merged: HistoryDataPoint[] = res.energy.map(e => {
                    ...
                    const s15m = res.solar15mForecast?.find(weather =>
                        new Date(weather.tsHourStart).getTime() === new Date(e.tsHourStart).getTime()
                    );
                    return {
                        ...e,
                        ...
                        solar15mImproved: s15m?.improvedSolarGeneration,
                        solar15mUnclipped: s15m?.unclippedSolarGeneration,
                ...
                setHasSolar15m(!!(res.solar15mForecast && res.solar15mForecast.length > 0));
            } catch (err) {
    ...
    const chartsToRender = useMemo(() => {
        ...
        if (hasSolar15m) {
            filtered.push({
                title: 'Solar Forecast Comparison (15m Forecast) (kWh)',
                unit: ' kWh',
                dataKeys: [
                    { key: 'solarKWH', color: 'var(--warning)', label: 'Actual Solar', type: 'area' },
                    { key: 'solar15mImproved', color: '#3b82f6', label: 'Forecast (15m)', type: 'line' },
                    { key: 'solar15mUnclipped', color: 'var(--text-muted)', label: 'Unclipped (15m)', type: 'line', strokeDasharray: '4 4' }
                ]
            });
        }

        return filtered;
    }, [settings, hasSolar1h, hasSolar15m]);
